...
# New CSS block for the float nav
nav_css = """
    /* ── 浮动导航 ── */
...
old_patterns = [
    r'\s*/* ── 浮动导航.*?float-nav-items\.open\{display:block\}\s*/',
    r'\.float-nav\{[^}]+\}',
    r'\.float-nav-btn\{[^}]+\}',
    r'\.float-nav-panel\{[^}]+\}',
    r'\.float-nav\.open \.float-nav-panel[^}]+\}',
    r'\.float-nav-panel::-webkit-scrollbar[^}]+\}',
    r'\.float-nav-header\{[^}]+\}',
    r'\.float-nav-header-title\{[^}]+\}',
    r'\.float-nav-section\{[^}]+\}',
    r'\.float-nav-section-btn\{[^}]+\}',
    r'\.float-nav-section-btn\.open[^}]+\}',
    r'\.float-nav-items\{[^}]+\}',
    r'\.float-nav-items\.open[^}]+\}',
    r'\.float-nav-item\{[^}]+\}',
    r'\.float-nav-item:hover[^}]+\}',
    r'\.float-nav-item\.active\{[^}]+\}',
]
...
def fix_file(fpath, is_daojia):
    with open(fpath, 'r', encoding='utf-8') as f:
        content = f.read()

    # Remove old float-nav CSS and JS
    ...
    # Replace old JS
    old_js = r'function toggleFloatNav\(\).*?</script>'
    content = re.sub(old_js, f'{js_code.strip()}</script>', content, flags=re.DOTALL)

    with open(fpath, 'w', encoding='utf-8') as f:
        f.write(content)

count = 0
for fpath, _, files in os.walk(base):
    for fname in files:
        if not fname.endswith('.html'):
            continue
        full = os.path.join(fpath, fname)
        is_daojia = 'daojia' in fpath
        try:
            fix_file(full, is_daojia)
