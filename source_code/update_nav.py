...
def update_file(path, nav_html, is_daojia):
    try:
    ...
    # Remove old float nav
    old_nav_pattern = r'<div class="float-nav"[^>]*>.*?</div>\s*</div>\s*(?=<footer>)'
    content = re.sub(old_nav_pattern, nav_html.strip(), content, flags=re.DOTALL)

    # Remove old JS functions
    old_js_pattern = r'function toggleFloatNav\(\).*?</script>'
    js_replacement = f'{js_snippet.strip()}</script>'
...
# Update zhesi/ files (except bao-pu-zi which already has the new nav)
count = 0
for fname in os.listdir(base):
    if not fname.endswith('.html'):
        continue
    fpath = os.path.join(base, fname)
    if fpath.endswith('bao-pu-zi.html'):
        continue  # already updated
    if update_file(fpath, zhuangzi_nav, False):
        count += 1
        print(f"Updated: {fname}")

# Update daojia/ files
for fname in os.listdir(daojia):
    if not fname.endswith('.html'):
        continue
    fpath = os.path.join(daojia, fname)
    if update_file(fpath, daojia_nav, True):
        count += 1
        print(f"Updated: daojia/{fname}")
