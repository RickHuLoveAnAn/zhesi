...
# The old CSS marker
OLD_CSS_START = "/* ── 浮动导航（两级） ── */"
OLD_CSS_START_2 = "/* ── 浮动导航 ── */"
...
for directory in [base, daojia]:
    ...
    for fname in os.listdir(directory):
        ...
        for marker in [OLD_CSS_START, OLD_CSS_START_2]:
            idx = new_content.find(marker)
            if idx == -1:
                continue
            # Find the next } that closes the last .float-nav-item.active rule
            # Count braces from marker to end of CSS block
            brace_count = 0
            in_block = False
            end_idx = idx
            for i, c in enumerate(new_content[idx:]):
                if c == '{':
                    in_block = True
                    brace_count += 1
                elif c == '}':
                    ...
                    if in_block and brace_count == 0:
                        end_idx = idx + i + 1
                        break
            new_content = new_content[:idx] + nav_css + new_content[end_idx:]
            break
